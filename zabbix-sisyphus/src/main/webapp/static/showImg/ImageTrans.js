...
ImageTrans.prototype = {
  ...
  reset: function() {
	if ( this._support ) {
		if(this._preConfig){
			...
			var ws = this._img.naturalWidth / this._clientWidth;
			var hs = this._img.naturalHeight / this._clientHeight;
			if(ws > 1 || hs > 1){
				if(ws > hs){
					this._x = this._y = 1 / ws;
				}else{
					this._x = this._y = 1 / hs;
				}
			}else{
				this._x = this._y = 1;
			}
  ...
  resize: function(width, height){
	this._clientWidth = width;//变换区域宽度
	this._clientHeight = height;//变换区域高度
	this._load();
	this.reset();
  },
  getScale: function(){
		...
		css3: {//css3设置
			...
			load: function(){
				...
				$$D.setStyle( img, {//居中
					...
					top: ( this._clientHeight - img.naturalHeight ) / 2 + "px",
					left: ( this._clientWidth - img.naturalWidth ) / 2 + "px",
					visibility: "visible"
				});
			},
			show: function() {
  ...
  scale: function () {
	...
	return function(zoom) {
		if( zoom ){
			...
			if ( hZoom && vZoom ) {
				var img = $(this._img);
				var imgTop = parseFloat(img.css("top"));
				var imgLeft = parseFloat(img.css("left"));
				var imgWidth = parseFloat(img.css("width"));
				var imgHeight = parseFloat(img.css("height"));
				var originX = imgLeft + imgWidth / 2;
				var originY = imgTop + imgHeight / 2;
				var x0 = this._clientWidth / 2;
				var y0 = this._clientHeight / 2;
				var offsetX0 = x0 - originX;
				var offsetY0 = y0 - originY;
				...
				var scale1 = this._x;
				var offsetX1 = offsetX0 * scale1 / scale0;
				var offsetY1 = offsetY0 * scale1 / scale0;
				var offsetX = offsetX0 - offsetX1;
				var offsetY = offsetY0 - offsetY1;
				img.css({"left": imgLeft + offsetX});
				img.css({"top": imgTop + offsetY});
			}
		}
	}
  }(),
